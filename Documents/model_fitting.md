...
In both methods the number of guesses per iteration is set by the user, the example being using 500. For the first iteration, the range of guesses is equally spaced between the current number of species published and a user-defined multiple of this number. The default multiple is 3. 
...
After the grid search, the gradient descent then begins from the best scoring pairing of a and b. For each step of gradient descent, the step maximising the reduction in the sum of squared-residuals is taken by calculating the partial derivative of this with respect to a and b and altering the parameters by this scaled by a step-size parameter. The step size parameter, alpha, is set by the user with the default being 0.01.
